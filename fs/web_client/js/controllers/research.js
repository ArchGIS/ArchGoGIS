  ...
  'show': function() {
    ...
    $.post('/hquery/read', query).success(function(researchData) {
      ...
      $.when(d4).done(function() {
        var counter = 0;
        _.each(data.monuments, function(monument, monid) {
          var query_excavation = JSON.stringify({
            "m:Monument": {"id": `${monument.id}`},
            "r:Research": {"id": `${id}`},
          ...
          $.post("/hquery/read", query_excavation).success(function(response) {
            response = JSON.parse(response);
            data.exc[monid] = response.exc;
            ...
            _.each(response.exc, function(exc, id) {
              data.placemarks.push({
                ...
                "pref": {
                  ...
                  "iconContent": `${monid+1}-${id+1}`
                }
              })
            })

            if (++counter == Object.keys(data.monuments).length) {
              d5.resolve();
              console.log("d5 complete");
            }
          })
        });
      ...
      $.post('/hquery/read', query_for_reports).success(function(researchD) {
        ...
        console.log(data);
        $.when(d1, d2, d3, d4, d5).done(function() {App.page.render('research_view', data)});
      });
    });
  }
}));